...
Mossland is a blockchain project that began as a metaverse connecting the real and virtual worlds and has grown into infrastructure for the AI civilization. Building on its virtual reality, augmented reality, and NFT-based entertainment roots, Mossland now develops agentic AI governance, AI-driven media, and Physical AI services — with Moss Coin (MOC) supporting ecosystem governance, coordination, and economic participation, which ties the ecosystem into an integrated economic system.
...
- [Disclosure](https://github.com/mossland/Disclosure-and-Materials)
...
#### Agora (Layer 2)
Agora는 [agora.moss.land](https://agora.moss.land/)에서 프로덕션으로 운영되는 DAO 거버넌스 플랫폼으로, 스택의 구속력 있는 결정 계층이자 기록의 원천입니다. 지갑 서명 한 번(EIP-712)으로 가스비 없이 MOC 보유량(스냅샷 블록 기준)에 비례해 투표하며, 결과는 누구나 독립적으로 재검증할 수 있습니다. 제안 라이프사이클, 포럼, SIWE(EIP-4361) 로그인을 제공하고, AI가 제안 초안을 작성(Gemini)하고 중립적 거버넌스 브리프를 생성(Claude Opus 4.8)합니다. 이 브리프 기능은 종료된 MAIT 서비스에서 이어받은 것입니다.
...
BRIDGE 2026은 사람의 제안이 아니라 **현실 신호(signal)에서 출발하는** 리얼리티 기반 거버넌스 프레임워크입니다. 온체인 활동·커뮤니티 지표·공개 데이터 같은 현실 신호를 수집·정규화하고(Reality Oracle), 거기서 이슈를 도출해(Inference Mining), 리스크·재무·커뮤니티·제품 관점의 다중 AI 에이전트가 심의한 뒤 하나의 '의사결정 패킷(Decision Packet)'으로 정리합니다(Agentic Consensus). 최종 결정은 MOC 보유자의 온체인 투표로 사람이 내리고(Human Governance), 승인된 결정은 실행 타임락으로 보호된 채 처리되며(Atomic Actuation), 그 결과는 KPI로 측정·기록되어 다음 거버넌스에 되먹임됩니다(Proof of Outcome). 거버넌스 스택의 Agora(사람 투표)와 종료된 MAIT의 AI 의사결정 개념을 하나의 순환 루프로 잇고, 상위 신호는 Alpha 같은 미디어 계층으로 흘러갑니다.

```mermaid
flowchart LR
    RO["🛰️ Reality Oracle<br/>현실 신호 수집·정규화"]
    IM["🔎 Inference Mining<br/>이슈 도출"]
    AC["🤖 Agentic Consensus<br/>다중 AI 심의 · 결정 패킷"]
    HG["🏛️ Human Governance<br/>MOC 보유자 사람 투표"]
    AA["⚙️ Atomic Actuation<br/>타임락 실행"]
    PO["📊 Proof of Outcome<br/>KPI 증명 · 되먹임"]
    RO --> IM --> AC --> HG --> AA --> PO
    PO -. 되먹임 .-> RO
```

핵심 원칙은 "**AI는 돕고, 사람이 결정한다**"이며, 완전 자율 재무 통제나 에이전트 단독 거버넌스는 명시적으로 범위에서 제외합니다. 저장소는 비전 명세와 동작하는 MVP를 함께 담은 모노레포(oracle: [bridge.moss.land](https://bridge.moss.land/)에 배포된 Next.js 14 + Express + SQLite 스택, nexus: NestJS 레퍼런스 구현)이며, 대시보드 수치가 대부분 목업인 실험/PoC 단계입니다. (BUSL-1.1 라이선스, Change Date 이후 Apache-2.0 전환)

- [Homepage (Experimental)](https://bridge.moss.land/)
- [GitHub](https://github.com/MosslandOpenDevs/bridge-2026)
