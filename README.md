...
- [GitHub](https://github.com/MosslandOpenDevs/signalmap)

### 2026 MOSS.AO (Agentic Orchestrator)
MOSS.AO는 다수의 AI 에이전트를 오케스트레이션하여 소프트웨어를 자율적으로 설계·구현하는 오픈소스 오케스트레이션 엔진(agentic-orchestrator)으로, 모스랜드 피지컬 AI(Physical AI) 생태계를 위한 인프라입니다. 로컬 LLM(Ollama)과 API 기반 모델을 하이브리드로 라우팅하며, 발산(divergence)·수렴(convergence)·계획(planning) 단계를 거치는 다중 에이전트 협업으로 Web3·피지컬 AI 서비스를 발굴하고 구현합니다. [ao.moss.land](https://ao.moss.land/)에서 실시간 파이프라인 대시보드가 운영 중입니다. (MIT 라이선스)
...
2026년 현재 MAIT의 AI 제안 초안 작성·거버넌스 브리프 기능은 Agora 거버넌스 플랫폼 안으로 통합되었으며, 독립 도메인 `mait.moss.land`는 `agora.moss.land`로 리다이렉트됩니다. (Agora 저장소 문서는 별도 DAO-MAIT 서비스를 'sunset'하고 그 AI 브리프 파이프라인을 Agora의 'Governance Pack'으로 흡수했다고 설명합니다.)

- [Homepage](https://agora.moss.land/) (`mait.moss.land` → `agora.moss.land` 리다이렉트)
...
1. **AI 기반 콘텐츠 생성**: 메타버스 내에서 사용자들이 쉽게 디지털 콘텐츠를 생성할 수 있도록 지원합니다.
2. **개인화된 사용자 경험**: 사용자의 활동 데이터를 기반으로 맞춤형 환경과 서비스를 제공합니다.
3. **자연어 처리 지원**: 메타버스 내에서 자연스러운 대화와 상호작용이 가능하도록 NLP 기술을 도입합니다.
4. **커뮤니티 관리 도구**: 커뮤니티 참여를 촉진하고 자동화된 관리를 통해 더 나은 사용자 경험을 제공합니다.

- [GitHub](https://github.com/mossland/MosslandAI)
...
- [Homepage](https://meta.moss.land/)
- [GitHub](https://github.com/mossland/mossverse)
- [소개자료](https://github.com/mossland/mossverse/raw/main/MosslandMetaverse_ko.pdf)
